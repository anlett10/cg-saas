...
function unreachableApi(): never {
    throw new TodoApiError(
        `Cannot reach API at ${env.API_URL}. Is \`bun run --filter @cg-saas/api dev\` running?`,
    );
}
...
export async function getTodos(): Promise<TodoView[]> {
    let result;
    try {
        result = await api.api.todos.get();
    } catch {
        unreachableApi();
    }
...
export async function createTodo(title: string): Promise<TodoView> {
    let result;
    try {
        result = await api.api.todos.post({ title });
    } catch {
        unreachableApi();
    }
...
export async function updateTodo(
    ...
    let result;
    try {
        result = await api.api.todos({ id }).patch(input);
    } catch {
        unreachableApi();
    }
...
export async function deleteTodo(id: number): Promise<TodoView> {
    let result;
    try {
        result = await api.api.todos({ id }).delete();
    } catch {
        unreachableApi();
    }
